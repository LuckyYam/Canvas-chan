...
export class GuessThePokemon {
    ...
    public build = async (): Promise<Buffer> => {
        const query = typeof this.pokemon === 'string' ? this.pokemon.toLowerCase() : this.pokemon.toString()
        let data
        try {
            data = await this.utils.fetch(`https://pokeapi.co/api/v2/pokemon/${query}`)
        } catch (error) {
    ...
    private createImage = async (pokemon: { name: string; image: string }): Promise<Buffer> => {
